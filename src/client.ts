import type { AnyEndpointDescriptor, ApiDefinition, ClientOptions, Codec, EndpointDescriptor, FailureKind, FailureResult, FetchResult, HttpResult, InferCodec, ParameterMap, ParameterSpec, RequestContext, SuccessResult } from "./types";
...
const failure = (kind: FailureKind, error: unknown, url: string, response?: Response): FailureResult => ({ ok: false, kind, status: response?.status ?? 0, error, headers: response?.headers ?? new Headers(), url, ...(response ? { response } : {}) });
...
function pathParameter(name: string, value: unknown, spec: ParameterSpec = {}): string {
  const style = spec.style ?? "simple"; const explode = spec.explode ?? false; const object = entries(value); const array = Array.isArray(value);
  if (style === "label") { const parts = object.length ? object.flatMap(([k, v]) => explode ? [`${encoded(k)}=${encoded(v)}`] : [encoded(k), encoded(v)]) : values(value).map(encoded); return `.${parts.join(explode ? "." : ",")}`; }
  if (style === "matrix") { if (object.length) return explode ? object.map(([k, v]) => `;${encoded(k)}=${encoded(v)}`).join("") : `;${encoded(name)}=${object.flatMap(([k, v]) => [encoded(k), encoded(v)]).join(",")}`; if (array && explode) return values(value).map((v) => `;${encoded(name)}=${encoded(v)}`).join(""); return `;${encoded(name)}=${values(value).map(encoded).join(",")}`; }
  if (style !== "simple") throw new TypeError(`Unsupported path parameter style: ${style}`);
  if (object.length) return object.flatMap(([k, v]) => explode ? [`${encoded(k)}=${encoded(v)}`] : [encoded(k), encoded(v)]).join(",");
  return values(value).map(encoded).join(",");
}
function queryParameter(output: URLSearchParams, name: string, value: unknown, spec: ParameterSpec = {}): void {
  const style = spec.style ?? "form"; const explode = spec.explode ?? true; const object = entries(value); const array = Array.isArray(value); const append = (key: string, item: unknown) => output.append(key, String(item));
  if (style === "deepObject") { if (!object.length) throw new TypeError(`deepObject query parameter ${name} must be an object`); for (const [key, item] of object) append(`${name}[${key}]`, item); return; }
  if (style === "spaceDelimited" || style === "pipeDelimited") { if (!array) throw new TypeError(`${style} query parameter ${name} must be an array`); append(name, values(value).join(style === "spaceDelimited" ? " " : "|")); return; }
  if (style !== "form") throw new TypeError(`Unsupported query parameter style: ${style}`);
  if (object.length) { if (explode) for (const [key, item] of object) append(key, item); else append(name, object.flatMap(([key, item]) => [key, item]).join(",")); return; }
  if (array && explode) for (const item of values(value)) append(name, item); else append(name, values(value).join(","));
}
function headerParameter(value: unknown, spec: ParameterSpec = {}): string {
  ...
  const object = entries(value); if (object.length) return object.flatMap(([key, item]) => spec.explode ? [`${key}=${String(item)}`] : [key, item]).join(",");
  return values(value).join(",");
}
const parameterSpec = (map: ParameterMap | undefined, name: string): ParameterSpec => { const value = map?.[name]; return value && typeof value === "object" && !("safeParse" in value) ? value : {}; };

export interface AdHocCall { url: string; method?: string; headers?: HeadersInit; query?: Record<string, unknown>; querySpec?: ParameterMap; body?: unknown; bodyCodec?: Codec; response?: Codec; responses?: Readonly<Record<number, Codec>>; errors?: Partial<Record<number | "default", Codec>>; signal?: AbortSignal; timeout?: number; endpoint?: EndpointDescriptor; operationId?: string }
export function createFetch(options: ClientOptions = {}) {
  return async (call: AdHocCall): Promise<FetchResult> => {
    ...
    try { if (call.query) { const query = new URLSearchParams(); for (const [key, value] of Object.entries(call.query)) if (value !== undefined) queryParameter(query, key, value, parameterSpec(call.querySpec, key)); const rendered = query.toString(); if (rendered) url += `${url.includes("?") ? "&" : "?"}${rendered}`; } } catch (error) { return failure("request", error, url); }
    ...
    const transport = options.fetch ?? globalThis.fetch; const terminal = async (context: RequestContext): Promise<FetchResult> => {
      ...
      const codec = response.ok ? call.responses?.[response.status] ?? call.response : call.errors?.[response.status] ?? call.errors?.default;
...
type InputParts<D> = D extends EndpointDescriptor<infer P, infer Q, infer H, infer B, any, any> ? { params: P; query: Q; headers: H; body: B } : never;
...
type EndpointInput<D extends AnyEndpointDescriptor> = Container<"params", InputParts<D>["params"], true> & Container<"query", InputParts<D>["query"]> & Container<"headers", InputParts<D>["headers"]> & Container<"body", InputParts<D>["body"], true> & { signal?: AbortSignal; timeout?: number };
...
export function createClient<A extends ApiDefinition>(api: A, options: ClientOptions = {}): ApiClient<A> {
  const execute = createFetch(options); const methods = Object.fromEntries(Object.entries(api.endpoints).map(([id, endpoint]) => [id, async (input: Record<string, any> = {}) => {
    let path = endpoint.path; try { for (const name of path.matchAll(/\{([^}]+)\}/g)) { const key = name[1]!; const value = input.params?.[key]; if (value === undefined) return failure("request", new TypeError(`Missing path parameter: ${key}`), path); path = path.replace(name[0], pathParameter(key, value, parameterSpec(endpoint.params, key))); } } catch (error) { return failure("request", error, path); }
    const extras = Object.keys(input.params ?? {}).filter((key) => !endpoint.path.includes(`{${key}}`)); if (extras.length) return failure("request", new TypeError(`Unexpected path parameter: ${extras[0]}`), path);
    const headers = new Headers(); try { for (const [key, value] of Object.entries(input.headers ?? {})) if (value !== undefined) headers.set(key, headerParameter(value, parameterSpec(endpoint.headers, key))); } catch (error) { return failure("request", error, path); }
    return execute({ url: path, method: endpoint.method, headers, query: input.query, querySpec: endpoint.query, body: input.body, bodyCodec: endpoint.body, responses: endpoint.responses, errors: endpoint.errors, signal: input.signal, timeout: input.timeout, endpoint, operationId: id });
  }])) as ApiClient<A>;
