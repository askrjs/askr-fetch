...
export interface ParameterSpec<T = unknown> { readonly validator?: Validator<T>; readonly style?: ParameterStyle; readonly explode?: boolean }
...
export interface EndpointDescriptor<P = undefined, Q = undefined, H = undefined, B = undefined, R extends Record<number, Codec> = Record<number, Codec>, E extends Partial<Record<number | "default", Codec>> = Partial<Record<number | "default", Codec>>> {
  ...
  readonly body?: Codec; readonly responses: Readonly<R>; readonly errors: Readonly<E>;
  readonly security?: readonly Readonly<Record<string, readonly string[]>>[]; readonly operationId?: string;
...
export interface ApiDefinition<E extends Record<string, AnyEndpointDescriptor> = Record<string, AnyEndpointDescriptor>> { readonly endpoints: E; readonly metadata?: ApiMetadata }
...
export interface RequestContext { readonly request: Request; readonly endpoint?: AnyEndpointDescriptor; readonly operationId?: string; readonly security?: AnyEndpointDescriptor["security"]; readonly attempt: number; readonly deadline?: number }
